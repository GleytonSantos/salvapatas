...
coords.forEach(function(coord) {
    ...
    var customIcon = L.divIcon({
        ...
        html: `
            ...
            </div> <div style="position: relative; top: 0; left: 0; width: ${radius * 2 }px; height: ${radius * 2 }px; background-color: rgba(95, 164, 255, 0.226); border-radius: 50%; border:1px solid rgba(17, 119, 252, 0.753) "></div>
        `,
...
function getCoords(){
    ...
    for(i=0;i<=savedData.length-1;i++){
        console.log(i)
        arr.push([savedData[i].latitude,savedData[i].longitude])
    }
    console.log(arr)
    return arr
}